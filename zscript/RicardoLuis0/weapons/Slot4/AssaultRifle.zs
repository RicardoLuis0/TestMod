...
class AssaultRifle : MyWeapon {
	...
	action State A_FireGun(){
		...
		A_FireBullets(invoker.getSpreadX(player.refire),invoker.getSpreadY(player.refire),1,7,"BulletPuff");
		A_PlaySound("AKFIRE");
	...
	action State A_PreReloadGun(){
		if(CountInv("AssaultRifleLoadedAmmo")==31||CountInv("Clip")==0){
	...
	action void A_ReloadGun(){
		...
		if(cur==0){
			invoker.ready=false;
			if(reserve>30){
				A_SetInventory("AssaultRifleLoadedAmmo",30);
			...
			if(reserve>reloadamt){
				...
				A_SetInventory("AssaultRifleLoadedAmmo",31);
			}else{
